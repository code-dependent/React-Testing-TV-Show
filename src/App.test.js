...
test("App Fretches Data and Displays Title description", async () => {
  mockFetchShow.mockResolvedValueOnce(st);
  const { queryAllByText, getByText } = render(<App />);
  await waitFor(() =>
    expect(queryAllByText(/stranger things/i)).toHaveLength(2)
  );
});
test("App Selection Form Works and Displays Seasons", async () => {
  mockFetchShow.mockResolvedValueOnce(st);
  const { queryByText, getByText } = render(<App />);
  await waitFor(() => expect(queryByText(/season 1/i)).toBeNull());
